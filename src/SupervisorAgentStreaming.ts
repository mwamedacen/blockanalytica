...
export class SupervisorAgentStreaming {
  private planningLLM;
  ...
  /**
   * Process a user query and stream responses from the appropriate agent(s) using LangGraph Supervisor
   */
  async *processQueryStream(userInput: string): AsyncGenerator<StreamResponse> {
    ...
    try {
      ...
      - When multiple data points need to be processed by an agent that handles one at a time,
        the plan will include multiple steps with the same agent name
